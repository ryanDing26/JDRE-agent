...
def evaluate(
    ...
    corpus_context_col: str = "CONTEXT",
    corpus_id_col: str = "DOC_ID",
    ...
    for idx, row in tqdm(it, desc="QA items"):
        ...
        try:
            idxs = rerank_hybrid(store, question, top_k=top_k or 10, final_k=final_k or 5)
        except Exception as e:
            print("[eval] Retrieval error:", e)
            idxs = []
        ...
        if not idxs:
            ...
            snippets, evidence_texts, evidence_ids = [], [], []
            for i in idxs:
                c = store.get_chunk(i)
    ...
    summary = {"num_examples": n, "avg_em": avg_em, "avg_f1": avg_f1, "avg_retrieved": avg_ret}
    print("[eval] SUMMARY")
...
def main():
    ...
    p.add_argument("--corpus_text_col", default="TEXT")
    p.add_argument("--corpus_context_col", default="CONTEXT")
    p.add_argument("--corpus_id_col", default="DOC_ID")
    p.add_argument("--qa", required=True)
    p.add_argument("--qa_q_col", default="question")
    p.add_argument("--qa_a_col", default="answer")
    p.add_argument("--out", default="eval_results.csv")
    p.add_argument("--max_examples", type=int, default=None)
    p.add_argument("--reflect", action="store_true")
    p.add_argument("--top_k", type=int, default=None)
    p.add_argument("--final_k", type=int, default=None)
    args = p.parse_args()
