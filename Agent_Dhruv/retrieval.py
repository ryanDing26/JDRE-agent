...
def combine_query_for_embedding(expanded_terms: Tuple[str, ...]) -> str:
    # de-duplicated readable string; SBERT handles it well
...
def offtopic_penalty(doc_text: str, q: str) -> float:
    ...
    if len(HEMO_TERMS.intersection(dtoks)) >= 2 and len(DERM_TERMS.intersection(dtoks)) == 0:
        return 1.0  # strongly hematology-only content → penalize for derm queries
    return 0.0
...
def mmr_select(
    ...
    """
    candidates: list of (doc_idx, final_score, primary_vec, text)
    Returns: selected doc indices (order matters).
    """
    if not candidates:
        return []
    # sort by score first; MMR builds on a good seed
    candidates = sorted(candidates, key=lambda x: x[1], reverse=True)
    selected = [0]  # index into candidates list
    if len(candidates) == 1:
...
def rerank_hybrid(
    ...
    """
    Multi-model semantic search → hybrid lexical/heuristic scoring → MMR.
    Returns final_k doc indices.
    """
    expanded = expand_query_terms(query)
    emb_query_str = combine_query_for_embedding(expanded)
    qvecs = store.mm.encode_query_multi(emb_query_str)  # per model
    # 1) semantic retrieval (ensemble across models)
    agg = store.mm.search_multi(qvecs, top_k=top_k)

    # 2) hybrid scoring (semantic + lexical + heuristics)
    ...
    for idx, embed_score in agg.items():
        ...
        final = ALPHA_EMBED * embed_score \
              + BETA_KEYWORD * kw_s \
              + GAMMA_PHRASE * (1.0 if phr > 0 else 0.0) \
              - DELTA_OFFTOPIC * off
        pvec = store.primary_vec(idx)
        if pvec is None:  # safety; should not happen
            continue
